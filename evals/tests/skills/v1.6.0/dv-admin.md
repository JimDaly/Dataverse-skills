...
> 3. **Recycle bin disable is PATCH, never DELETE.** `PATCH statecode=1, statuscode=2, isreadyforrecyclebin=false`. DELETE enqueues async opt-out and orphans per-entity configs ΓÇö see [`references/recycle-bin.md`](references/recycle-bin.md).
...
**Four mechanisms ΓÇö pick based on where the setting lives:**

| Mechanism | Use for | How |
|---|---|---|
| **PAC CLI** (`pac org update-settings` / `list-settings`) | Columns on the `organization` entity (audit, plugin trace, typeahead, quick find, canvas/flow solutions, email validation, audit retention) | `--name <column> --value <value>` ΓÇö accepts any org column, not just the legacy audit ones |
| **Python SDK ΓÇö OrgDB XML** | Keys inside the `orgdborgsettings` XML blob (MCP, search, Fabric, Work IQ, TDS endpoint, attachment security, ownership, address records, block unmanaged, delete users, Excel AI) | Read XML ΓåÆ parse ΓåÆ modify ΓåÆ PATCH whole blob back on `organizations({id})` |
| **Python SDK ΓÇö recyclebinconfigs** | Recycle bin on/off + retention days | CREATE/PATCH `recyclebinconfigs` entity record |
| **Python SDK ΓÇö settingdefinition + organizationsettings** | App-level / plan-level security role toggles | Look up `settingdefinition` by `uniquename` ΓåÆ CREATE or PATCH `organizationsettings` row with `value` |
...
## Allowed settings ΓÇö hard allowlist

The 37 PPAC toggles below are the only ones this skill may read or update (35 unique backend keys ΓÇö `SearchAndCopilotIndexMode` and `auditretentionperiodv2` each cover two toggles). Examples that are **out of scope and must be refused** per the safety rule above: `sessiontimeoutinmins`, `isautosaveenabled`, `maxuploadfilesize`, `inaborttimeoutinmins`, `IsArchivalEnabled`, `IsShadowLakeEnabled`, `EnableActivitiesFeatures`. Do not run `pac org list-settings` without `--filter`, and do not dump the whole `orgdborgsettings` XML for a non-allowlisted setting.

### Organization entity columns ΓÇö use PAC CLI (14)

Use `pac org list-settings --filter <column>` to read, `pac org update-settings --name <column> --value <value>` to write. PAC CLI accepts any column on the organization entity ΓÇö not just the legacy audit ones.
...
| 12 | Minimum number of characters to trigger typeahead | `lookupcharactercountbeforeresolve` | int (0ΓÇôMAX_INT, null = feature off) |
...
### OrgDB XML keys ΓÇö use Python SDK on `orgdborgsettings` blob (17)
...
**`SearchAndCopilotIndexMode`** is one int (0ΓÇô3) that encodes two UI toggles ΓÇö Dataverse search ├ù M365 Copilot search:
...
- **Destructive / stateful** (bulk delete schedule/cancel/pause/resume, settings updates, recycle bin toggle, role assignment, self-elevate, retention set) ΓÇö preview in prose: what's changing, new value, target environment(s). Use placeholders (`<ENV_URL>`) for unknowns and ask for missing values in the same turn. Skip the raw CLI block.
- **Read-only** (list-settings, show job, read OrgDB / recycle bin status) ΓÇö one-sentence prose preview is enough.
...
- Γ¥î "The command requires approval. Please confirm to pause the job."
- Γ£à "I'll pause bulk delete job `<job-id>` on the active environment. Confirm to proceed."

**Audit status across N environments (read-only, multi-call):**
- Γ¥î Sequential `pac org fetch` per env, or starting with Python/SDK because it "feels like a query."
- Γ£à "I'll run `pac org list-settings --filter audit` in parallel across all N environments (one `&`-batch, single `wait`)."

## How to Read or Update Org Settings

**Org settings always go through `pac org list-settings` / `pac org update-settings`** ΓÇö never raw Web API, FetchXML, PowerShell, or Python for org columns. Use `--filter <substring>` for category reads in one call. Multi-environment work runs in parallel via `&` + `wait` in ONE bash call.
...
**Multi-environment ΓÇö parallel in ONE bash call:**
```bash
...
If `pac org list-settings` fails for a setting, that setting is NOT an org column ΓÇö check the mechanism routing in the four-mechanism table at the top, then use the appropriate Python pattern. Do NOT fall back to Web API, PowerShell, FetchXML, or `pac org fetch` for org columns.

---

## Prerequisites

- PAC CLI **latest .NET Framework build** ΓÇö `pac data bulk-delete` and `pac data retention` are only in the .NET Framework build, not the `dotnet tool` cross-platform version. Check with `pac help` (look for "Version: x.x.x (.NET Framework ...)"); if it shows `.NET 10` or `.NET 8`, run `pac install latest && pac use latest` to switch.
- Authenticated (`pac auth create`), active profile (`pac auth list`), and System Administrator privilege on the target environment.

## Multi-Environment Operations ΓÇö Always Parallel

The same `&` + `wait` pattern from `list-settings` applies to every multi-env operation (`update-settings`, `bulk-delete`, etc.) ΓÇö N backgrounded calls in ONE bash call, never sequential or `for` loops.

---

## Common Mistakes ΓÇö Do NOT Use These
...
| `--fetchxml` | `-fx` | No | FetchXML filter. **See the hard-stop rule below ΓÇö if omitted, ALL records in the table are deleted.** |
...
1. **Refuse until the user explicitly acknowledges** with the word ALL (or ALL RECORDS) **and** the entity logical name ΓÇö e.g., `"yes, delete ALL records in contact"`. Bare `"yes"` rejected.
2. **Disambiguate vague asks** ("clean up old emails") ΓÇö propose a FetchXML filter with date / statecode / owner conditions before showing any command.
3. **Empty-filter FetchXML doesn't bypass the gate** ΓÇö `<filter/>` or `<filter><condition><value/></condition></filter>` still targets every record.
...
**Args:** `--name <column>` (required), `--value <value>` (required; `true`/`false` for bool, int for option sets), `--environment <url>` (optional). Allowed columns are the 14 listed in [Allowed settings](#allowed-settings--hard-allowlist) ΓÇö anything else is out of scope.

**Batch workflow:** `pac admin list` ΓåÆ filter targets ΓåÆ confirm ΓåÆ run all `update-settings` calls in parallel (`&` + `wait`) ΓåÆ render summary table.

---

## Advanced Settings (Python SDK ΓÇö PAC CLI Cannot Handle These)

OrgDB, recycle bin, and settings-definition overrides each need raw Web API or the Python SDK ΓÇö PAC CLI does not cover them. The four-mechanism routing table at the top of this skill ([┬º Skill](#skill-environment-admin--bulk-delete-retention-org-settings-orgdb-recycle-bin)) maps each setting to its mechanism. Sub-sections below summarise the patterns and link to the references for full Python.

### OrgDB Settings (orgdborgsettings XML)

OrgDB settings live as PascalCase XML elements inside the `orgdborgsettings` column of the `organizations` entity. PAC CLI cannot read or write these ΓÇö use raw Web API.

**Quick reference:** `GET /organizations?$select=organizationid,orgdborgsettings` ΓåÆ parse XML with `xml.etree.ElementTree` ΓåÆ modify or `SubElement` ΓåÆ `PATCH /organizations({id})` with the serialized XML.
...
Recycle bin settings live in the `recyclebinconfigs` entity (NOT `orgdborgsettings`). PAC CLI cannot manage them ΓÇö use raw Web API.
...
- **Disable:** PATCH `statecode=1, statuscode=2, isreadyforrecyclebin=false`. **Do NOT DELETE** ΓÇö it enqueues async opt-out and orphans per-entity configs.
