---
name: dv-metadata
description: Dataverse schema authoring via the Python SDK and Web API ΓÇö tables, columns, relationships, forms, and views. Use when the user wants to define or evolve the data model ΓÇö add a column, create a table, set up a lookup, customize a form, or build a view.
---

# Skill: Metadata ΓÇö Making Changes

**Before the first metadata change in a session:**
1. **Confirm the target environment** with the user ΓÇö see the Multi-Environment Rule in dv-overview.
2. **Confirm the solution** ΓÇö ask "What solution should these components go into?" If `SOLUTION_NAME` is in `.env`, confirm it. If no solution exists yet, **you MUST ask the user** for the solution name and publisher prefix before creating anything. The publisher prefix is **permanent** ΓÇö it cannot be changed after components are created with it.
...
Then query existing publishers and show them ΓÇö the user may want to reuse one:

```python
# Publisher discovery + solution creation ΓÇö use SDK (never raw Web API).
...
The exported XML is generated by Dataverse itself and is always valid. Hand-written XML is fragile ΓÇö a single incorrect attribute or missing element causes an import failure with an opaque error.
...
- **Idempotent Table Creation** ΓÇö check-first pattern for re-runnable scripts
- **Alternate Keys** ΓÇö required for upsert; create immediately after each table
- **Metadata Propagation Delays and Lock Contention** ΓÇö phased creation to avoid lock errors
...
**Never name a regular column with an `Id` suffix** (e.g., `prefix_CountryId`). Dataverse auto-generates a navigation property with the `Id` suffix when you create a lookup ΓÇö if a regular column with that name exists, lookup creation fails with a schema name collision.

- WRONG: `prefix_DepartmentId` (int) ΓÇö collides with auto-generated lookup
...
**Web API approach (needed for column types the SDK doesn't support ΓÇö e.g., currency with precision, memo with custom max length):**
...
**SDK approach ΓÇö simple lookup (preferred):**
...
**SDK approach ΓÇö full control over 1:N relationship:**
...
**SDK approach ΓÇö many-to-many relationship:**
...
**After creating a lookup ΓÇö the @odata.bind navigation property:**
...
The MCP server and Python SDK do not support forms or views ΓÇö both require raw Web API calls (`urllib`).
...
- **Publish:** `POST /api/data/v9.2/PublishXml` with `<importexportxml><entities><entity>...` ΓÇö required for forms to take effect.
...
- Do not use `python -c` for GUID generation on Windows ΓÇö write a `.py` file.
...
```
Or check `solutions/<SOLUTION_NAME>/Other/Solution.xml` after the first pull ΓÇö look for `<CustomizationPrefix>`.
...
- **`labelid` is also a GUID** ΓÇö not a human-readable string.
- **Subgrid controls require a valid `<ViewId>`** ΓÇö must be the GUID of an existing SavedQuery. Create the view first.
- **Cell, section, tab, and control IDs must all be unique** across the entire form.
- **Control `classid` values** ΓÇö see the classid table above.

**Tip:** Create forms in the maker portal and pull via `pac solution export` ΓÇö use the pulled XML as a template for programmatic creation.
...
| `0x80048d19` | Invalid property in payload. A field name doesn't match any column on the table. | Check logical column names ΓÇö use `EntityDefinitions(LogicalName='...')/Attributes` to verify. |
| `0x80040237` | Schema name already exists. | Verify the column/table exists before creating a new one ΓÇö it may have been created by a previous timed-out call. |
...
After creating tables / columns / alternate keys, Dataverse runs internal metadata operations (index build, cache propagation) for 3ΓÇô30 seconds. Submitting another metadata operation while these run causes lock-contention errors.

**Mitigation ΓÇö phased creation, not interleaved.** Create ALL tables ΓåÆ wait 15ΓÇô30s ΓåÆ create ALL alternate keys ΓåÆ wait 15ΓÇô30s ΓåÆ create ALL lookups. Do NOT interleave operations on the same table.
...
**After every metadata session, perform the pull-to-repo sequence** ΓÇö see dv-overview "After Any Change: Pull to Repo" for the full export/unpack/commit commands.
...
When creating tables programmatically (e.g., a schema setup script that may be re-run), use a check-first pattern ΓÇö query `client.tables.get()` before creating. This is explicit, avoids masking unrelated errors, and lets you branch logic based on whether the table was created or reused:
...
- Use a check-first pattern with `client.tables.get_alternate_keys(table)` to skip keys that already exist ΓÇö see `references/alternate-keys.md` for the `ensure_alternate_key` helper.
- Index creation is **async** ΓÇö for large tables, poll `client.tables.get_alternate_keys(table)` until `status == "Active"` before using.
...
- **Self-referential lookups** (e.g., Parent ΓåÆ same table) must be added via `update_table` after the table is created.
- **Metadata cache delays.** After `create_table`, call `describe_table` before `update_table` to force cache refresh.
- **Column name normalization.** Spaces in column names become underscores: `"Specialty Area"` ΓåÆ `cr9ac_specialty_area`. Always verify with `describe_table`.