---
name: dv-overview
description: Tool routing and cross-cutting rules for Dataverse work ΓÇö which skill applies to which task, environment-confirmation, and pull-to-repo. Use when the user mentions Dataverse, Dynamics 365, Power Platform, or CRM; this skill picks the specialist (dv-connect / dv-data / dv-metadata / dv-query / dv-solution / dv-admin / dv-security) for the request.
---

# Skill: Overview ΓÇö What to Use and When

This skill provides cross-cutting context that no individual skill owns: tool capabilities, UX principles, and the skill index. Per-task routing is handled by each skill's WHEN/DO NOT USE WHEN frontmatter triggers ΓÇö not duplicated here.

---

## Hard Rules ΓÇö Read These First
...
- If EITHER is missing: **Automatically run the connect flow** (see the `dv-connect` skill). Do NOT ask the user whether to initialize ΓÇö just do it. Do not create your own `.env`, `requirements.txt`, `.env.example`, or auth scripts. The `dv-connect` skill handles all of this.
...
### 1. Python Only ΓÇö No Exceptions

All scripts, data operations, and automation MUST use **Python**. This plugin's entire toolchain ΓÇö `scripts/auth.py`, the Dataverse SDK, all skill examples ΓÇö is Python-based.
...
### 2. MCP ΓåÆ SDK ΓåÆ Web API (in that order)
...
- **Writes:** Γëñ10 records ΓåÆ use MCP directly. 10+ records ΓåÆ use Python SDK (`dv-data`).
- **Reads:** Simple filter, small result set (no paging needed) ΓåÆ use MCP. Multi-page iteration, DataFrame loading, aggregation, or queries hitting SQL limits (DISTINCT, HAVING, subqueries) ΓåÆ use Python SDK (`dv-query`).
...
**If MCP can't handle it** (bulk operations, large reads, schema creation, multi-step workflows, analytics, or MCP tools aren't available), **use the Python SDK ΓÇö not raw HTTP.** This is the most common mistake agents make.

**SDK checklist ΓÇö evaluate EVERY time you write a script:**
- Creating/updating/deleting records? ΓåÆ `client.records.create()`, `.update()`, `.delete()` ΓÇö see `dv-data`
- Bulk record operations? ΓåÆ `client.records.create(table, [list_of_dicts])` ΓÇö see `dv-data`
- Querying or filtering records? ΓåÆ `client.records.get(table, select=[...], filter="...")` ΓÇö see `dv-query`
- Aggregation (top-N, sum, count by group, "most/least")? ΓåÆ Single-table: `$apply` server-side aggregation (raw Web API). Cross-table: `client.dataframe.get()` with `$select` + `pd.merge()` ΓÇö see `dv-query`. Do NOT load all records without `$select` and aggregate in Python.
- Loading data into pandas? ΓåÆ `client.dataframe.get(table)` ΓÇö see `dv-query`
- Single record by GUID? ΓåÆ `client.records.get(table, guid)` ΓÇö see `dv-query`
- Creating tables, columns, relationships? ΓåÆ `client.tables.create()`, `.add_columns()`, `.create_lookup_field()` ΓÇö see `dv-metadata`
- Creating publishers or solutions? ΓåÆ `client.records.create("publisher", {...})`, `client.records.create("solution", {...})` ΓÇö see `dv-solution`

**Before using `from auth import get_token` or `import requests`:** check whether the operation is in the Raw Web API list below. If it is not in that list ΓÇö the SDK supports it ΓÇö use `from auth import get_client` instead. Using raw HTTP for SDK-supported operations is the most common off-rails mistake.
...
- Record CRUD or bulk operations ΓÇö use `client.records.create()`, `.update()`, `.delete()` (see `dv-data`)
- Publisher or solution creation ΓÇö use `client.records.create("publisher", {...})` (see `dv-solution`)
- Table, column, or relationship creation ΓÇö use `client.tables.create()`, `.add_columns()`, `.create_lookup_field()` (see `dv-metadata`)
- Querying records ΓÇö use `client.records.get()` or `client.query.builder()` (see `dv-query`)
...
Each skill documents a specific, tested sequence of steps. Follow them. If a skill says "use the Python SDK," use the Python SDK ΓÇö do not substitute a raw HTTP call, a different library, or a different language. If a skill says "run this command," run that command ΓÇö do not invent an alternative.

**Do NOT introspect the SDK** with `dir()`, `inspect.signature()`, or `help()` to discover APIs. The skills document the exact methods and parameters to use. If a method call fails with `AttributeError`, the installed SDK version may not have it ΓÇö check the version note in the skill and fall back to the documented alternative. Introspecting the SDK wastes time and leads to using deprecated or internal APIs.
...
From that single prompt, the agent should orchestrate the full sequence: check if the workspace is initialized ΓåÆ create metadata via Web API ΓåÆ write and deploy a C# plugin ΓåÆ pull the solution to the repo. No skill names, no commands ΓÇö just intent.
...
**Before the FIRST operation that touches a specific environment** ΓÇö creating a table, deploying a plugin, pushing a solution, inserting data ΓÇö you MUST:
...
## Tool Capabilities ΓÇö Which Tool for Which Job
...
| --- | --- | --- |
| **MCP Server** | Data CRUD (create/read/update/delete records), table create/update/delete/list/describe, column add via `update_table`, keyword search, single-record fetch | Forms, Views, Relationships, Option Sets, Solutions. **Note:** table creation may timeout but still succeed ΓÇö always `describe_table` before retrying. Run queries sequentially (parallel calls timeout). Column names with spaces normalize to underscores (e.g., `"Specialty Area"` ΓåÆ `cr9ac_specialty_area`). **SQL limitations:** The `read_query` tool uses Dataverse SQL, which does NOT support: `DISTINCT`, `HAVING`, subqueries, `OFFSET`, `UNION`, `CASE`/`IF`, `CAST`/`CONVERT`, or date functions. For analytical queries that need these (e.g., finding duplicates, unmatched records, filtered aggregates), use `$apply` (single-table aggregation) or `client.dataframe.get()` with pandas (cross-table) ΓÇö see `dv-query`. **Bulk operations:** MCP `create_record` creates one record at a time. For 10+ records, use the Python SDK `CreateMultiple` instead ΓÇö see `dv-data`. |
...
| **Web API** | Everything ΓÇö forms, views, relationships, option sets, columns, table definitions, unbound actions, `$ref` association | Nothing (full MetadataService + OData access) |
| **PAC CLI** | Solution export/import/pack/unpack, environment create/list/delete/reset, auth profile management, plugin updates (`pac plugin push` ΓÇö first-time registration requires Web API), user/role assignment (`pac admin assign-user`), solution component management | Data CRUD, metadata creation (tables/columns/forms) |
...
**Tool priority (always follow this order):** MCP for simple reads/queries (small result set, no paging) and Γëñ10 record writes ΓåÆ Python SDK for bulk reads, scripted writes, bulk operations, and analysis ΓåÆ Web API for operations the SDK doesn't cover (forms, views, option sets, `$apply`, N:N `$expand`) ΓåÆ PAC CLI for solution lifecycle. Schema creation (tables/columns/relationships) ΓåÆ SDK via `dv-metadata`. MCP tools not in your tool list? ΓåÆ Load `dv-connect` to set them up (see below).

**Volume guidance ΓÇö writes:** MCP `create_record` for 1-10 records. For 10+ records, use `dv-data` (`client.records.create(table, list_of_dicts)`) ΓÇö it uses `CreateMultiple` internally. **Note:** the SDK does not chunk automatically; for large datasets, chunk in your script starting at 1,000 and adapt up or down based on success (see `dv-data` for the adaptive pattern).

**Volume guidance ΓÇö reads:** MCP `read_query` for simple filters and small result sets (no paging needed). For bulk reads (multi-page iteration, all-records loads, DataFrame handoff), use `dv-query` SDK ΓÇö it streams pages automatically and avoids MCP SQL limitations. For aggregation queries (`$apply`), use the Web API directly (see `dv-query`).

Note: The Python SDK is in **preview** ΓÇö breaking changes possible.

### MCP Availability Check

If the user's request involves MCP ΓÇö either explicitly ("connect via MCP", "use MCP", "query via MCP") or implicitly (conversational data queries where MCP would be the natural tool) ΓÇö check whether Dataverse MCP tools are available in your current tool list (e.g., `list_tables`, `describe_table`, `read_query`, `create_record`).
...
4. After MCP is configured, **stop here** ΓÇö the session must be restarted for MCP tools to appear. Remind the user to resume the session without losing context (Claude Code: `claude --continue`; Cursor: reload the window with Ctrl+Shift+P ΓåÆ "Developer: Reload Window"; Copilot: reopen the Copilot panel). Do not proceed with SDK. Wait for the user to restart.
...
1. This is a SDK fallback case ΓÇö use the Python SDK to answer the question. Do not block the user.
2. After answering, offer: "MCP would handle this conversationally ΓÇö want me to set it up?"

The distinction matters: explicit MCP request ΓåÆ block and set up MCP. Implicit/conversational question ΓåÆ answer with SDK, offer MCP setup.
...
| Script | Purpose |
| --- | --- |
| `auth.py` | Azure Identity token/credential acquisition ΓÇö used by all other scripts and the SDK |
...
When running in a Unix-style shell on Windows (Git Bash is the default for Claude Code on Windows; Cursor and Copilot may use PowerShell ΓÇö the same rules apply, adapt path/quoting syntax as needed):
...
3. If no solution exists yet, **load the `dv-solution` skill** and follow its publisher discovery + solution creation flow. Use the SDK ΓÇö **never raw Web API** ΓÇö to create publisher and solution records:

```python
# Quick reference ΓÇö full pattern with publisher discovery is in dv-solution
