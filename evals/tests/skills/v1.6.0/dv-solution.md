---
name: dv-solution
description: Dataverse solution lifecycle ΓÇö create, export, import, promote across environments, and validate deployments. Use when the user wants to package customizations, deploy to another environment, or move work between dev / test / prod.
---
...
**Use the Python SDK for publisher and solution record creation ΓÇö not raw HTTP.** Publishers and solutions are standard Dataverse tables. `client.records.create()` and `client.records.get()` handle auth, pagination, and error handling automatically, avoiding the URL encoding, header boilerplate, and GUID-parsing bugs that raw `urllib` calls introduce.

### Step 1: Find or Create the Publisher

Every solution belongs to a publisher. The publisher's `customizationprefix` (e.g., `contoso`, `sa`, `lit`) is prepended to every custom table, column, and relationship schema name. **This prefix is effectively permanent** ΓÇö existing components keep their prefix forever, even if you change the publisher later.
...
**Discovery flow ΓÇö always run this before creating a publisher:**
...
if publishers:
    ...
    # No custom publisher exists ΓÇö ASK THE USER for prefix
    # "What publisher prefix should I use? (e.g., 'contoso', 'sa', 'lit' ΓÇö 2-8 lowercase chars)"
...
- The prefix must match any tables already created in the solution ΓÇö you cannot mix prefixes.
...
If the header was misspelled or the solution doesn't exist, components will be created in the default solution instead ΓÇö silently. Always verify.
...
> **Confirm the target environment before exporting or importing.** Run `pac auth list` + `pac org who`, show the output to the user, and confirm it matches the intended environment. Developers work across multiple environments ΓÇö do not assume.
...
Delete the zip ΓÇö the unpacked folder is the source:
```
...
After importing a solution, verify that components are live. Use the Python SDK to check directly ΓÇö no external scripts needed.
...
```python
# Web API required ΓÇö SDK does not support N:N $expand
import os, sys, urllib.request, json
sys.path.insert(0, os.path.join(os.getcwd(), "scripts"))
from auth import get_token, get_plugin_headers, load_env  # get_token + get_plugin_headers ΓÇö SDK can't do this
...
- Large solutions (Sales, Customer Service) can take 10ΓÇô20 minutes to import. Be patient and poll rather than re-importing.
