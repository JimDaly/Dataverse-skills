---
name: dv-connect
description: One-step setup for a Dataverse environment ΓÇö installs tools, authenticates, registers the MCP server, and writes `.env`. Use when starting a new project, switching environments, fixing authentication, or troubleshooting an MCP connection that won't come up.
---

# Skill: Connect

One-step connection to Dataverse. Handles tool installation, authentication, environment selection, workspace initialization, MCP configuration, and verification ΓÇö all idempotently. Each step checks if it's already done and skips if so.

> **Environment-First Rule** ΓÇö All metadata (solutions, columns, tables, forms, views) and plugin registrations are created **in the Dynamics environment** via API or scripts, then pulled into the repo. Never write or edit solution XML by hand to create new components.
...
Before touching anything, check whether this workspace is already connected to a Dataverse environment. This matters a lot on `claude --continue` or any re-run ΓÇö repeating it on an already-configured workspace overwrites `.env`, re-registers MCP, and wastes time.

Run these checks in order. If **all four pass**, skip straight to Step 7 (final verification) and stop there.

1. **`.env` is present and complete** ΓÇö file exists at the workspace root and contains non-empty values for `DATAVERSE_URL`, `TENANT_ID`, and `MCP_CLIENT_ID`
2. **MCP is registered** ΓÇö `.mcp.json` (Claude Code) or the equivalent Copilot / Cursor config file has a `dataverse-*` server entry pointing at the `DATAVERSE_URL` from `.env`
3. **Both auth surfaces match `.env`** ΓÇö `dataverse auth who` shows a profile whose `Environment Url` matches `DATAVERSE_URL`, AND `pac org who` against a PAC profile for the same URL succeeds. (DV CLI auth covers Connect / Data / Query / Metadata / MCP / Python; PAC auth covers `dv-solution` and `dv-admin`. Both are front-loaded at connect time so neither prompts later.)
4. **Python SDK is importable and current** ΓÇö `python -c "from PowerPlatform.Dataverse.client import DataverseClient; import pandas; from importlib.metadata import version; v=version('PowerPlatform-Dataverse-Client'); assert v>='0.1.0b9', f'SDK {v} is outdated, need >=0.1.0b9'"` exits 0
...
**If any check fails:** Proceed through the normal flow (Steps 1ΓÇô7), but still use each step's own skip condition. A partially-configured workspace doesn't need a full redo ΓÇö e.g., if only `.env` and MCP are missing but tools and auth are fine, start at Step 2 or Step 3.

---

## Step 1: Ensure tools are installed

Check each tool independently ΓÇö **do not use fail-fast parallel execution.** If one tool check fails, continue checking the others so you can report all missing tools at once. See [tools-setup.md](references/tools-setup.md) for installation commands and platform-specific notes.
...
.NET SDK is needed for PAC CLI but NOT for the Dataverse CLI (the npm package bundles its own runtime). Node.js powers the Dataverse CLI npm package (`@microsoft/dataverse`), which is used as the MCP proxy and for scripted data plane actions. Azure CLI is used as a fallback for environment discovery when PAC CLI isn't available (see [mcp-configuration.md](references/mcp-configuration.md) Step 3b). GitHub CLI is not needed for connecting ΓÇö it's used later for ALM/CI/CD scenarios (see `dv-solution`).
...
> **Auth tool choice.** Two tools, two AAD apps, two caches ΓÇö front-load both at connect:
>
> 1. **`dataverse auth create`** (app `0c412cc3-ΓÇª`) covers DV CLI + MCP + Python.
...
```
dataverse auth create --environment <url>          # interactive (WAM broker on Windows ΓåÆ no browser tab)
...
PAC uses its own AAD app, so a separate sign-in is required for `dv-solution` and `dv-admin`. Do it now ΓÇö user signs in twice back-to-back, no later surprises.
...
> 1. **Interactive login (recommended)** ΓÇö Sign in via browser. No app registration needed. Token stays cached across sessions.
> 2. **Service principal (for CI/CD)** ΓÇö Uses CLIENT_ID and CLIENT_SECRET from an Azure app registration.

Write `.env` directly ΓÇö do not instruct the user to create it:
...
Also set plugin attribution variables for User-Agent tagging. **Fill in the two literals below from your own context** ΓÇö you (the agent) loaded this plugin, so you already know both values:

- `PLUGIN_VERSION` ΓÇö the `version` field of your loaded plugin manifest (e.g. `"1.5.0"`). At runtime, `auth.py` re-reads this from the live manifest via host env vars; this `.env` entry is a fallback for offline cases.
- `AGENT` ΓÇö your host identity, one of: `claude-code`, `copilot`, `cursor`, `codex`, or `unknown`. Must match an entry in `_ALLOWED_AGENTS` in `auth.py` ΓÇö if you don't recognize your host, use `unknown`.

```python
# Substitute these two literals from your loaded plugin context.
# Do NOT leave the angle-bracket placeholders ΓÇö replace with real values.
...
**If any fail:**
- `dataverse auth who` fails ΓåÆ re-run Step 2.
- `pac org who` fails ΓåÆ re-run Step 2b.
- `python scripts/auth.py` prints a device-code URL ΓåÆ DV CLI cache missing/wrong tenant; re-run Step 2 and confirm `msal` + `msal-extensions` are installed (`pip show msal msal-extensions`).
- Other Python error ΓåÆ check SDK install and `.env`.
...
6. Handle admin consent and allowlist ΓÇö prefer `dataverse mcp allow <MCP_CLIENT_ID>` over the portal (one-time per tenant/environment)

**Plugin attribution for MCP:** This plugin uses the **stdio proxy** transport (`npx @microsoft/dataverse mcp <url>`) ΓÇö the CLI runs as a local subprocess and proxies requests to the Dataverse MCP HTTP endpoint. When registering it, include `DATAVERSE_OPERATION_CONTEXT` in the env block so the CLI appends it to its User-Agent on outbound requests to `/api/mcp`. Build the value from `.env`:
...
> Γ£à Dataverse MCP server `dataverse-{orgid}` configured in `~/.codex/config.toml`. **Restart Codex** (CLI) or reload the Codex IDE to load the MCP tools.
...
**Check 1: `claude mcp list` (or Copilot equivalent) shows Γ£ô Connected**
```
claude mcp list
```
This proves the MCP server process starts and speaks the MCP protocol. It does NOT by itself prove that data operations work ΓÇö authentication, environment allowlisting, and endpoint reachability are only exercised on the first real tool call.
...
- If Check 1 fails (server not Γ£ô Connected): the MCP server itself cannot start. Re-run Step 6 and check that `npx`/Node.js are installed and the MCP registration succeeded.
...
**Diagnostic ΓÇö `--validate` (for failure investigation only):**
```
...
- **GA / Production endpoint** ΓÇö `{DATAVERSE_URL}/api/mcp`. This is the one the plugin actually uses at runtime.
- **Preview endpoint** ΓÇö `{DATAVERSE_URL}/api/mcp_preview`. Opt-in per environment; not used by the plugin.
...
> Γ£à Connected to Dataverse at `{DATAVERSE_URL}`. Tools installed, authenticated, MCP live.
>
> You can now:
...
This plugin's skill files are natively loaded by both **GitHub Copilot CLI** and **Claude Code CLI** when installed as a plugin. No manual context-loading is needed ΓÇö both agents discover and invoke skills automatically.
