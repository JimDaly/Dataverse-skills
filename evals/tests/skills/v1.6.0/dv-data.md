---
name: dv-data
description: Record-level CRUD and bulk operations via the Python SDK ΓÇö create, update, delete, upsert, CSV import, multi-table foreign-key loads, AI-generated sample data. Use when the user wants to write, modify, seed, or import data records into Dataverse tables.
---

# Skill: Data ΓÇö Create, Update, Delete, and Bulk Import

> **This skill uses Python exclusively.** Do not use Node.js, JavaScript, or any other language for Dataverse scripting. If you are about to run `npm install` or write a `.js` file, STOP ΓÇö you are going off-rails. See the overview skill's Hard Rules.
...
**PyPI package:** `PowerPlatform-Dataverse-Client` (this is the only official one ΓÇö do not use `dataverse-api` or other unofficial packages)
**Status:** Preview ΓÇö breaking changes are possible
...
## Before Writing ANY Script ΓÇö Check MCP First

**If MCP tools are available** (`create_record`, `update_record`) and the task is Γëñ10 records, **use MCP directly ΓÇö no script needed.** Only write a Python script when the task requires: bulk operations (10+ records), data transformation, retry logic, CSV import, or operations the SDK supports that MCP cannot (upsert, file uploads). Sequential MCP tool calls are not "multi-step logic" ΓÇö use MCP for those.

## SDK-First Rule

**If an operation is in the "supports" list below, you MUST use the SDK ΓÇö not `urllib`, `requests`, or raw HTTP.**

**Correct import** (always preceded by `sys.path.insert` in a full script ΓÇö see Setup below):
```
...
`get_token()` and `requests` exist ONLY for operations the SDK does not support (forms, views, `$apply`, N:N `$expand`, unbound actions) ΓÇö see **dv-query** and **dv-metadata**.
...
- Record reads within write workflows (e.g., lookup resolution) ΓÇö for standalone queries see **dv-query**
...
- Forms (FormXml) ΓÇö see **dv-metadata**
- Views (SavedQueries) ΓÇö see **dv-metadata**
- Global option sets ΓÇö see **dv-metadata**
- N:N record association (`$ref` POST) ΓÇö use raw Web API (`POST /api/data/v9.2/<entity>(<id>)/<nav-property>/$ref`)
- N:N `$expand` ΓÇö see **dv-query**
- `$apply` aggregation ΓÇö see **dv-query**
...
# Do not modify the context value ΓÇö it is a closed schema for server-side
...
|---|---|---|---|
| **Structural** (columns) | LogicalName ΓÇö always lowercase | `new_name`, `new_priority` | Record payload keys |
| **Navigation** (lookups) | Navigation Property Name ΓÇö case-sensitive, matches `$metadata` | `new_AccountId` | `@odata.bind` keys |
...
guid = client.records.create("new_ticket", {
    "new_name": "Ticket 001",
    "new_priority": 100000002,          # choice column ΓÇö integer value, not string
...
- Value is `"/<EntitySetName>(<guid>)"` ΓÇö e.g., `"/accounts(<guid>)"`
- If you just created the lookup column, wait 5ΓÇô10 seconds before inserting. Metadata propagation delays cause "Invalid property" errors.
...
**Important:** The SDK sends all records in a single POST to `CreateMultiple`. It does **not** chunk automatically. Dataverse has no fixed record count limit ΓÇö the constraints are payload size and request timeout (SDK default: 120s for POST). For larger datasets, you **must** chunk in your script. The `bulk_upsert` and `bulk_create` helpers below use adaptive chunking: start at 1,000, double on success (up to 4,000), halve on payload/timeout failure, and cap at the last successful size. Tables with few columns can handle larger chunks than tables with many columns.
...
To create or update records from a pandas DataFrame, use the `client.dataframe` namespace. This is documented in **dv-query** (alongside `client.dataframe.get()`) but is a write operation ΓÇö include it in your data write workflow:

```python
# Update records ΓÇö DataFrame must include the primary key column
client.dataframe.update("opportunity", df_updates, id_column="opportunityid")

# Create records ΓÇö returns a Series of new GUIDs
...
Idempotent ΓÇö re-running the same import does not create duplicates. The alternate key must be defined on the table first ΓÇö see **dv-metadata**.
...
> **For imports that may be re-run** (most real-world cases), use `UpsertItem` with alternate keys instead of `create()` ΓÇö see [`references/multi-table-fk-import.md`](references/multi-table-fk-import.md). The `create()` pattern here is for one-shot loads only.

| Volume | Tool | Why |
|---|---|---|
| 1ΓÇô10 records | MCP `create_record` | Simple, no script |
...
# SDK sends all in one POST ΓÇö chunk to avoid payload/timeout limits
...
for row in rows:
    ...
    if not customer_guid:
        print(f"Skipping row ΓÇö unknown email: {row['customer_email']}")
        continue
    records.append({
...
3. Build source-ID ΓåÆ GUID maps by querying back (upsert doesn't return GUIDs).
4. Repeat per dependency level ΓÇö Level 1 needs Level 0's maps for `@odata.bind`.

For the full pattern ΓÇö adaptive `bulk_upsert` helper, composite-key handling, post-import verification, and the first-time `bulk_create` variant ΓÇö see [`references/multi-table-fk-import.md`](references/multi-table-fk-import.md).
...
- **Alternate key columns must NOT also appear in the record body** ΓÇö `UpsertMultiple` fails.
- **Catch per-table failures** in the executor ΓÇö one table failing must not kill the others.
...
try:
    ...
    # 400 ΓÇö bad field name, @odata.bind format, or missing required field
    # 403 ΓÇö check security roles
    # 404 ΓÇö table or record not found
    # 429 ΓÇö rate limited; SDK retries automatically, reduce batch size if persistent
```
...
- **ASCII only** in `.py` files ΓÇö curly quotes and em dashes cause `SyntaxError` on Windows.
- **No `python -c` for multiline code** ΓÇö write a `.py` file instead.
...
Generate realistic sample records inline ΓÇö schema-driven, table-agnostic, PII-safe defaults (`@example.com` emails, `555-01xx` phones).

**Quick reference:** confirm environment + count + table ΓåÆ query `EntityDefinitions(LogicalName='<table>')/Attributes?$filter=AttributeOf eq null` for required columns ΓåÆ dispatch by `AttributeType` (String / Memo / Integer / DateTime / Picklist / etc.) ΓåÆ `client.records.create()` (use `CreateMultiple` for count >= 10).
...
- `UserLocalizedLabel` may be null ΓÇö dereference safely.

### Confirmation-flow examples

**Generate N sample records (destructive ΓÇö preview the snippet, ask for env):**
- Γ¥î "Which environment should I target? Please provide the Dataverse URL."
- Γ£à "I'll run the Sample Data Generation snippets with `TABLE=\"contact\"`, `COUNT=20`. Uses `CreateMultiple`, `.example.com` emails, `555-01xx` phones, against the active `pac auth list` environment. Confirm to proceed, or specify a different environment."

**Sample data on a custom entity (schema unknown ΓÇö prose is enough):**
- Γ¥î "I need more info about the entity. What are the required fields?"
- Γ£à "Custom entity ΓÇö I'll query `EntityDefinitions` for `cr123_project` to discover required columns, then generate 5 records inline mapping each column to a generator by `AttributeType` and call `client.records.create(\"cr123_project\", records)`. Confirm to proceed, or tell me a different count."