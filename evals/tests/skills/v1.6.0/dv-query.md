---
name: dv-query
description: Bulk reads, multi-page iteration, and analytics over Dataverse data via the Python SDK and Web API. Use when the user wants to read, list, filter, aggregate, group, join, or analyze records ΓÇö including pandas DataFrame workflows and notebook exploration.
---

# Skill: Query ΓÇö Read and Analyze Dataverse Records
...
**All reads use the SDK ΓÇö not `urllib`, `requests`, or raw HTTP.** This is the same rule as dv-data's SDK-First Rule, applied to reads. If you find yourself writing `urllib.request` or `get_token()` for a query, STOP ΓÇö the SDK handles it. The only exceptions are `$apply` aggregation and N:N `$expand`, documented below.
...
| "export this data" / bulk extract | **`client.dataframe.get()`** with `select=` | Direct to DataFrame ΓåÆ CSV |
...
**Key principle:** Let the server do the work. For single-table aggregation, use `$apply` ΓÇö it runs server-side and returns only grouped results. For cross-table questions, use `client.dataframe.get()` with minimal `$select` on each table, then `pd.merge()` ΓÇö the merge itself is sub-second; the bottleneck is network transfer, which `$select` minimizes.
...
## SQL Queries ΓÇö `client.query.sql()`

`client.query.sql()` uses the Dataverse Web API `?sql=` parameter ΓÇö a **limited SQL subset** (same limitations as MCP `read_query`). It does NOT support GROUP BY, JOINs, HAVING, DISTINCT, or subqueries. Results are capped at ~5,000 rows.
...
|---|---|---|---|
| **Structural** (columns) | LogicalName ΓÇö always lowercase | `new_name`, `new_priority` | `$select`, `$filter`, `$orderby` |
| **Navigation** (lookups) | Navigation Property Name ΓÇö case-sensitive, matches `$metadata` | `new_AccountId` | `$expand` |
...
`client.records.get()` is the primary read method ΓÇö works on all SDK versions (b6+). It returns a page iterator for multi-record queries and a single Record for by-GUID fetch. **Always use `select=` to limit columns.**
...
`client.records.get()` returns a page iterator ΓÇö always iterate pages and then records within each page. Each record is a `Record` object that supports dict-like access: `r["column"]`, `r.get("column")`, `r.keys()`. Do not use `r.data.get()` ΓÇö use `r.get()` directly.
...
for page in client.records.get("opportunity",
    ...
    for r in page:
        ...
        print(f"{r['name']} ΓÇö {account_name}")
```

**You MUST pass `include_annotations`** ΓÇö without it, the `Prefer: odata.include-annotations` header is not sent and formatted values are not in the response. Use `"*"` for all annotations or the specific annotation name above.

Formatted values are available for lookup, choice, status, and owner fields.

---

## $expand ΓÇö Resolve Lookup to Full Related Record
...
for page in client.records.get("opportunity",
    ...
    for r in page:
        ...
        print(f"{r['name']} ΓÇö {account.get('name', 'Unknown')}")
```

Always use nested `$select` inside `$expand` ΓÇö without it, Dataverse returns every column on the related entity, which wastes bandwidth and memory.
...
For aggregations and many-to-many expansion, the SDK doesn't have direct support ΓÇö use raw Web API. See [`references/web-api-advanced.md`](references/web-api-advanced.md) for full code samples.

**Quick reference:**
- **`$expand` on N:N relationships:** `GET /<entitySet>?$expand=<n:n_nav>($select=...)` ΓÇö single page only; follow `@odata.nextLink` for >5,000 results.
...
- **Cross-table aggregation:** `$apply` only works within one entity set. Use `client.dataframe.get(entity, select=[...])` per table ΓåÆ `pd.merge()` ΓåÆ `groupby()`. Always pass `select=`; without it transfers 10-20├ù more data.

## QueryBuilder ΓÇö Fluent Query API (SDK b8+)
...
| 404 | Table logical name not found | Check spelling ΓÇö use `client.tables.get("<name>")` to verify |
