...
Auth priority (first match wins):
  ...
  4. Device code flow (legacy fallback) — interactive on first login,
...
def _get_credential():
    ...
    order matches the module docstring: service principal → certificate →
    shared DataverseCLI cache → device-code fallback.
    """
    global _credential
    ...
    # Path 2: Certificate-based service principal (non-interactive).
    ...
    # Path 3: Shared DataverseCLI MSAL cache (populated by `dataverse auth
    ...
    # Path 4: Legacy device-code fallback with this script's own cache.
